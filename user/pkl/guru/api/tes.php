...
for ($i = 0; $i < $totalCatatan; $i++) {
    ...
    $tot = $a + $totalCatatan;
    for ($b = $a; $b < $tot; $b++) {
        $excel->getActiveSheet()->SetCellValue('E' . $b, $saran[$i++]);
    }
}
...
$file->save(str_replace('.php', '.xlsx', __FILE__));

echo " Click here for gerate xlsx file <a href='tes.xlsx'>clicking me</a>";
