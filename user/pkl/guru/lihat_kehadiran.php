                ...
                <div class="card-body p-3 pb-3 mb-3" style="overflow-x:auto;">
                    <table class="table mt-3 text-center" style="min-width: 100%">
                        ...
                        if (!isset($_POST['filter'])) {
                            $getOldest = mysqli_query($conn, "SELECT MIN(MONTH(tgl_kehadiran)) as oldest_month FROM kehadiran 
                            JOIN siswa ON kehadiran.nis = siswa.nis
                            JOIN guru ON siswa.nip_pembimbing = guru.nip
                            JOIN kehadiran_detail ON kehadiran.id_kehadiran_detail = kehadiran_detail.id_kehadiran_detail
                            WHERE siswa.nis = '$nis'");
                            $fetchOldest = mysqli_fetch_array($getOldest);
                            echo "Bulan : " . bulanIndo($fetchOldest['oldest_month']);

                            $getKegiatan = mysqli_query($conn, "SELECT *, MONTH(tgl_kehadiran) as bulan FROM kehadiran
                            JOIN siswa ON kehadiran.nis = siswa.nis
                            JOIN guru ON siswa.nip_pembimbing = guru.nip
                            JOIN kehadiran_detail ON kehadiran.id_kehadiran_detail = kehadiran_detail.id_kehadiran_detail
                            WHERE siswa.nis = '$nis' AND MONTH(tgl_kehadiran) = '" . $fetchOldest['oldest_month'] . "'");

                            $no = 1;
                            while ($value = mysqli_fetch_array($getKegiatan)) { ?>
                                <tr>
                                    <td><?php echo $no++; ?></td>
                                    <td><?php echo $value['tgl_kehadiran']; ?></td>
                            ...
                            JOIN guru ON siswa.nip_pembimbing = guru.nip
                            JOIN kehadiran_detail ON kehadiran.id_kehadiran_detail = kehadiran_detail.id_kehadiran_detail
                            WHERE siswa.nis = '$nis' AND MONTH(tgl_kehadiran) = '$bln'");
...
<?php foreach ($getKegiatan as $key => $value) { ?>
    <div class="modal fade" id="edit<?php echo $value['id_kehadiran']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="api/edit_kehadiran.php" method="POST">
                        <div class="form-group">
                            <input type="hidden" name="id" value="<?php echo $value['id_kehadiran']; ?>" class="form-control">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="kegiatan" value="<?php echo $value['kehadiran']; ?>" class="form-control" readonly>
                        </div>
                        <div class="form-group">
                            <label>Tanggal Kegiatan</label>
                            <input type="date" name="tgl" value="<?php echo $value['tgl_kehadiran']; ?>" class="form-control" readonly>
                        </div>
                        <div class="form-group">
